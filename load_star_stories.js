...
async function loadStories() {
  try {
    console.log('Loading STAR stories into database...');

    for (const story of stories) {
      try {
        const response = await axios.post(`${API_BASE}/stories`, story);
        console.log(`✓ Created story: "${story.title}" (ID: ${response.data.id})`);
      } catch (error) {
        console.error(`✗ Failed to create story "${story.title}":`, error.response?.data?.error || error.message);
      }
    }
